...
function _inherite (Parent, Child) {
  Child.prototype = createObject(Parent.prototype) // 将超类型的原型指定为一个临时的空构造函数的原型，并返回构造函数的实例。
  Child.prototype.constructor = Child
  Child.__proto__ = Parent
  // 此时由于构造函数内部为空, 所以返回的实例也不会自带实例属性, 然后为这个对象重新指定constructor为Child，并将其赋值给Child的原型
  // 将超类型构造函数的实例作为子类型原型的目的，同时没有一些从Child继承过来的无用原型属性
}
