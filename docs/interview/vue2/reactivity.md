...
在vue3当中则是使用proxy来实现响应式数据，使用Proxy 的好处是它可以完美的监听到任何方式的数据改变，唯一的缺点是兼容性的问题，因为 Proxy 是 ES6 的语法。
...
## 在data上面定义了一个$name属性，可以通过Vue.$name拿到吗？
...
## 给响应式数据添加标识为什么不可以直接使用value.__ob__ = this?
...
## 定义响应式数据的时候在defineReactive的set方法内部，不直接value[key]赋值？
