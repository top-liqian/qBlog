...
```js
let state = { count: 0 }; // app.innerHTML = state.count;
let active; // 1.将数据变成响应式数据
...
watcher(() => { app.innerHTML = state.count; });
watcher(() => { console.log(state.count) });
```
...
defineReactive(state);
function render(){ app.innerHTML = state }
render();
state.push(4)
```